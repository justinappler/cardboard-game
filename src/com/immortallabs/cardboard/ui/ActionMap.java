 ...
 *          2/09 Version 1.5 Forgot to add a version history for release 1. This
 *          was committed sometime in February. This included the code for mouse
 *          clicks and basic menu options. (New game and exit). - 3/04/09
 *          Version 1.6 Finished code for release two. Includes keybindings, and
 *          new menu items.
 */
public class ActionMap extends JComponent implements ActionListener,
        KeyListener, MouseListener
{
    ...
    public static enum EVENT
    {
        ...
        /** KeyBinds Event: Display the key binding */
        KEY_BINDS
        {
            GameState.ACTION getAction() throws NoActionException
            {
    ...
    private EnumSet<EVENT> playerSet =
            EnumSet.of(EVENT.ONE, EVENT.TWO, EVENT.THREE, EVENT.FOUR);

    /**
     * All events corresponding to game modes
     */
    private EnumSet<EVENT> gameModes =
            EnumSet.of(EVENT.BEGINNER_GAME, EVENT.COMPETITIVE_GAME,
                EVENT.MULTIPLAYER_GAME, EVENT.SINGLEPLAYER_GAME, EVENT.ONE,
    ...
    public void processEvent(EVENT interEvent)
    {
        try
        {
            ...
            if (interEvent.equals(EVENT.PAUSE) && !noAction)
            {
            ...
            else if (interEvent.equals(EVENT.UNPAUSE) && noAction)
            {
                // SET noAction to false
                noAction = false;
            ...
            else if (interEvent.equals(EVENT.HINT))
            {
                // GET the setsBoard from gameWindow
                SetsBoard setsBoard = gameState.getBoard();
                // GET the inlay size before hint is called
                int before = setsBoard.getInlaySize().intValue();
                // Move the selector for a hint
                gameWindow.moveSelector(GameWindow.DIRECTION.HINTDIR);
                // CALL processAction with ACTION's HINT_SELECTED
                gameState.processAction(GameState.ACTION.HINT);
                // IF the hint is not the third hint
                if (before != 2 && !gameWindow.hasMessage)
                    // repaint gameWindow
                    gameWindow.repaint();
            }
            else if (!noAction)
        ...
        if (interEvent.equals(EVENT.REMOVE_INLAY_POS_ONE)
                || interEvent.equals(EVENT.REMOVE_INLAY_POS_TWO))
            gameWindow.repaint();
    }
    ...
    public void processEvent(CardBoardButton clickedButton)
    {
        ...
        // ELSE IF clickedButton's getType equals BUTTONTYPE's OK
        if (clickedButton.getType() == CardBoardButton.BUTTONTYPE.OK)
        {
            gameWindow.repaint();
        }
        // ELSE IF clickedButton's getType equals BUTTONTYPE's PAUSE
    ...
    public void keyPressed(KeyEvent keyEvent)
    {
        ...
        //GIVE focus to actionMap
        this.requestFocusInWindow();
        //IF key is an alt
        if (keyEvent.equals(KeyEvent.VK_ALT))
        {
            //FOCUS on menu
            gameWindow.menu.getMenuBar().requestFocusInWindow();
        }
        // ELSE IF interEvent doesn't equal null
        else if (interEvent != null)
        {
    ...
    public void actionPerformed(ActionEvent actionEvent)
    {
        ...
        if (actionEvent.getActionCommand().equals(EVENT.SOUND_TOGGLE.name()))
        {
            ...
            if (pref.getPreference(CardBoardPreferences.PREF.SOUNDS).equals(
                "OFF"))
            {
                // SET the sound preference to on
                pref.setPreference(CardBoardPreferences.PREF.SOUNDS, "ON");
            }
            // ELSE turn it off
            else
            {
                // SET the sound preference to off
                pref.setPreference(CardBoardPreferences.PREF.SOUNDS, "OFF");
            }
        }
        // ElSE key bindings is selected
        else if (actionEvent.getActionCommand().equals(EVENT.KEY_BINDS.name()))
        {
            Object[][] data =
                    new Object[][] {
                            { "Hint", "h" },
                            { "Pausing the game", "p" },
                            { "Asking for three more cards", "t" },
                            { "Moving a seleced card to the inlay", "enter" },
                            { "Moving the selected card up", "up arrow" },
                            { "Moving the selected card down", "down arrow" },
                            { "Moving the selected card left", "left arrow" },
                            { "Moving the selected card right", "right arrow" },
                            { "Moving inlay card 1 back to the table", "1" },
                            { "Moving inlay card 2 back to the table", "2" },
                            { "Player one to buzz in", "left shift" },
                            { "Player two to buzz in", "space" },
                            { "Player three to buzz in", "right control" },
                            { "Player four to buzz in", "numpad" }, };
            Object[] columns = new Object[] { "Action", "Key Bind" };
            ...
            JOptionPane.showMessageDialog(gameWindow, jsp, "Key Bindings",
                ...
                .getActionCommand())))
        {
            ...
            pref.setPreference(CardBoardPreferences.PREF.PLAYERS, actionEvent
                    .getActionCommand());
            // CALL process action new multiplayer game
            gameState.processAction(GameState.ACTION.NEW_MULTIPLAYER_GAME);
        }
        ...
        // IF the the action is to start a new game and game is paused
        if (gameModes.contains(EVENT.valueOf(actionEvent.getActionCommand())))
        {
            // set actionMap to allow actions
            noAction = false;
            // remove the pause screen from game window-
            gameWindow.removeButton(CardBoardButton.BUTTONTYPE.UNPAUSE);

            // Get rid of multiplayer score if applicable
            gameWindow.displayBuzzedPlayer(null);
        }
