...
java -cp ../resources/emma.jar emma instr -ip ./CardBoardButton/classes/ -m overwrite -filter -CardBoardButton* -filter -*GameState* -filter -*ActionMap* -filter -*CardBoardResource* -filter -*Deck* -filter -*Card* -filter -*CardBoardMessage* -filter -*Scoree* -filter -*SetsBoard* -filter -*SoundPlayer* -filter -*Timer* -filter -*CardBoardMenu* -filter -*CardBoardUI* -filter -*CardGraphic* -filter -*GameWindow* -filter -*KeyBindings* -filter -*ScoresGraphic* -filter -*TextUI* -filter -*TimerGraphic* -out CardBoardButton_Int.em
