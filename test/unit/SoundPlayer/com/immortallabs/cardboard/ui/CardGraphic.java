...
public class CardGraphic extends JComponent implements Runnable
{
     ...
     */
    private static final int[] kInlayX = {10, 10, 10};

    /**
     * The y position of the inlay spots for the cards
     */
    private static final int[] kInlayY = {10, 200, 400};
     ...
     */
    private static final int[] kSpace = {40, 30, 20, 10};
    /**
     * The x offset on the game board of the cards
     */
    private static final int kOffX = 300;
    /**
     * The y offset on the game board of the cards
     */
    private static final int kOffY = 100;
    /**
     ...
     */
    private static final int kDelay = 10;
            ...
            Natural column, GameWindow gamewindow, Point drawLocation)
    {
        ...
        theme = "com/immortallabs/cardboard/resources/";
    ...
    /**
     * Animates a card from its current position to a desired point. Uses a
     * seperate thread to animate this CardGraphic without losing the response
     * of the GUI.
     * 
     * @param inlaySpot
     *            the spot on the inlay to animate to
     * @see Point
     */
    public void animateTo(Point animatePoint)
    {
        // IF currently in the middle of an animation
        if (animate != null)
        {
            // KILL that thread
            animate = null;
        ...
        dstPoint = animatePoint;
        // SET dx to (the point - the card graphic's current x position)
        dx = dstPoint.x - drawLocation.x;
        // SET dy to (the point - the card graphic's current y position)
        dy = dstPoint.y - drawLocation.y;

        // IF dx >= SPEED
        if (((int) Math.abs(dx)) >= kSpeed)
        {
            // dx = dx / SPEED
            dx = dx / kSpeed;
        }
        // IF dy >= SPEED
        if (((int) Math.abs(dy)) >= kSpeed)
        {
            // dy = dy / SPEED
            dy = dy / kSpeed;
        }
    ...
    /**
     * The method to run on a seperate thread to animate this CardGraphic's
     * position.
     */
    public synchronized void run()
    {
        ...
        while (animate != null)
        {
            ...
            int distFromX =
                    ((int) Math.abs(drawLocation.x - dstPoint.x));
            // distFromY=absolute(cards y position - dstPoint y)
            int distFromY =
                    ((int) Math.abs(drawLocation.y - dstPoint.y));
            // IF distFromX > absolute(dx)
            if (distFromX > Math.abs(dx))
            {
                // ADD dx to the cards x position
                drawLocation.setLocation(drawLocation.x + dx, drawLocation.y);
            }
            // ELSE cards x position = x coordinate of dstPoint
            else
            {
                drawLocation.setLocation(dstPoint.x, drawLocation.y);
            }
            // IF distFromY > absolute(dy)
            if (distFromY > Math.abs(dy))
            {
                // ADD dy to the cards y position
                drawLocation.setLocation(drawLocation.x, drawLocation.y + dy);
            }
            // ELSE cards y position = y coordinate of dstPoint
            else
            {
                drawLocation.setLocation(drawLocation.x, dstPoint.y);
            }
            ...
            if (drawLocation.equals(dstPoint))
            {
                
                // STOP animation
                animate = null;
                // CALL actionMapInst.CardClicked
                actionMapInst.cardClicked(this);
            }
            // END WHILE
        }
    }
