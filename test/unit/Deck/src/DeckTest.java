...
public class DeckTest extends TestCase 
{
	...
	public void testBeginnerGetCardsLeft()
	{
		assertEquals(d1.getCardsLeft(), new Natural(27));
		d1.deal();
		assertEquals(d1.getCardsLeft(), new Natural(26));
		for(int i = 0; i < 10; i++)
			d1.deal();
		assertEquals(d1.getCardsLeft(), new Natural(16));
		for(int i = 0; i < 15; i++)
			d1.deal();
		assertEquals(d1.getCardsLeft(), new Natural(1));
		d1.deal();
		assertEquals(d1.getCardsLeft(), new Natural(0));
		d1.deal();
		assertEquals(d1.getCardsLeft(), new Natural(0));
		
	}
	
	public void testNormalGetCardsLeft()
	{
		assertEquals(d2.getCardsLeft(), new Natural(81));
		for(int i = 80; i > 1; i--)
		{
			d2.deal();
			assertEquals(d2.getCardsLeft(), new Natural(i));
		}
		d2.deal();
		assertEquals(d2.getCardsLeft(), new Natural(1));
		d2.deal();
		assertEquals(d2.getCardsLeft(), new Natural(0));	
	}
	
	public void testPhysicalShuffleCheck()
	{
		System.out.println("this is a test to have someone visually" +
				" check if an unshuffled deck and a shuffled deck match" +
				" up, which they should not. It is very unlikely that three" +
				" cards will match up when both decks have been shuffled randomly.\n");
